...
/**
  ...
  @module sonatribe-ember-infinity/mixins/route
  @extends Ember.Mixin
*/
const RouteMixin = Ember.Mixin.create({
	...
	*/
	...
	/**
	 * Whether or not to use the skip / take
	 * method of paging
	 * @type {Boolean}
	 * @default  false
	 */
	useSkipTakeMethod: false,

	/**
	 * Name of the "skip" param in the
	...
	/**
	 * Path of the "total records" param in
	 * the HTTP response
	 * @type {String}
	 * @default "meta.total"
	 */
	totalRecordsParam: 'meta.total',
	...
	/**
	  ...
	  @default false
	*/
	_canLoadMore: Ember.computed('_totalPages', 'currentPage', 'useSkipTakeMethod', '_totalRecords', 'skip', 'take', function() {

		if (!this.get('useSkipTakeMethod')) {
			let totalPages = this.get('_totalPages');
	...
	*/
	...
	/**
	 ...
	 @private
	 @method _buildParams
	 @param {Number} nextValue the page number for the current request
	 @return {Object} The query params for the next page of results
	 */
	_buildParams(nextValue) {
		if (!this.get('useSkipTakeMethod')) {
			const pageParams = {};
			pageParams[this.get('perPageParam')] = this.get('_perPage');
			pageParams[this.get('pageParam')] = nextValue;
			...
			const pageParams = {};
			pageParams[this.get('skipParam')] = nextValue;
			pageParams[this.get('takeParam')] = this.get('_take');
	...
	_nextPageLoaded(newObjects) {
		...
		if (!this.get('useSkipTakeMethod')) {
			...
			const totalRecords = newObjects.get(this.get('totalRecordsParam'));
			this.set('_totalRecords', totalRecords);
	...
	_notifyInfinityModelUpdated(newObjects) {
		...
		if (!this.get('useSkipTakeMethod')) {
			...
			Ember.run.scheduleOnce('afterRender', this, 'infinityModelUpdated', {
				lastPageLoaded: this.get('currentPage'),
				totalRecords: this.get('_totalRecords'),
				newObjects: newObjects
			});
		}
	},
